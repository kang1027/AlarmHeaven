...
<body>
	...
	String originquestion = request.getParameter("originquestion");
	String quiztype = request.getParameter("quiztype");
	String question = request.getParameter("question");
	String answer = request.getParameter("answer");
	String rank = request.getParameter("rank");
	try(Connection conn = JDBCConn.DBCPConnection();
		PreparedStatement stmt = conn.prepareStatement("update testquiz set quiztype=?, question=?, answer=?where question=?")){
		stmt.setString(1, quiztype);
		stmt.setString(2, question);
		stmt.setString(3, answer);
		stmt.setString(4, originquestion);
		stmt.executeUpdate();
		%>
			<script>
			alert(
