...
try (Connection conn = JDBCConn.DBCPConnection();
		PreparedStatement stmt = conn.prepareStatement("insert into usermusic values(?,?)")) {

	// request.getRealPath("상대경로") 를 통해 파일을 저장할 절대 경로를 구해온다.
	// 운영체제 및 프로젝트가 위치할 환경에 따라 경로가 다르기 때문에 아래처럼 구해오는게 좋음
	String uploadPath = request.getRealPath("/uploadMusic\\" + session.getAttribute("id") + "music");
